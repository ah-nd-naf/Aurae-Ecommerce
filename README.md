...
*   **Security & OTP**: `bcryptjs` (passwords), `jsonwebtoken` (auth tokens), `Google Apps Script Mailer` (Gmail API email OTP)
...
```env
# Google Apps Script Mailer (used for sending signup validation OTPs)
GMAIL_SCRIPT_URL="https://script.google.com/macros/s/..."
GMAIL_SCRIPT_TOKEN="your-custom-secure-token"
...
*   **Configured Variables**: Database connection URL (`DATABASE_URL`), JWT configs, Google Apps Script Mailer (`GMAIL_SCRIPT_URL` and `GMAIL_SCRIPT_TOKEN`), SSLCommerz keys, and CORS settings (`FRONTEND_URL` and `BACKEND_URL`).
